...
void evaluatePositionANDTurn(int* leftPeakLoc, int* rightPeakLoc, signed DiffLine[]){
	...
	}else if(leftPeakFound == 1 && rightPeakFound ==1){
	//found both
		dif = *rightPeakLoc - *leftPeakLoc;
		dif = dif/2;
		CalculatedPosition = *leftPeakLoc+dif;
	}else if(leftPeakFound == 0 && rightPeakFound ==1){
	//found right
		CalculatedPosition = *rightPeakLoc - halfTrack;
	}else if(leftPeakFound == 1 && rightPeakFound ==0){
	//found right
		CalculatedPosition = *leftPeakLoc + halfTrack;
	}
  ...
  if(CalculatedPosition<64) {
		if (CalculatedPosition>62){
			TurnPercent = 5;

		}
		else if (CalculatedPosition>60){
			TurnPercent = 10;
		}
		else if (CalculatedPosition>58){
			TurnPercent = 15;
		}
		else if (CalculatedPosition>56){
			TurnPercent = 25;
		}
		else if (CalculatedPosition>54){
			TurnPercent = 40;
		}
		else if (CalculatedPosition>52){
			TurnPercent = 50;
		}
		else if (CalculatedPosition>50){
			TurnPercent = 60;
		}
		else if (CalculatedPosition>48){
			TurnPercent = 70;
		}
		else if (CalculatedPosition>46){
			TurnPercent = 80;
		}
		else if (CalculatedPosition>44){
			TurnPercent = 90;
		}
		else {
			TurnPercent = 100;
		}
		TurnLeftPercent(TurnPercent);
	}
	else{
		if (CalculatedPosition<66){
						TurnPercent = 10;
		}
		else if (CalculatedPosition<66){
						TurnPercent = 15;
		}
		else if (CalculatedPosition<68){
						TurnPercent = 25;
		}
		else if (CalculatedPosition<70){
						TurnPercent = 40;
		}
		else if (CalculatedPosition<72){
						TurnPercent = 50;
		}
		else if (CalculatedPosition<74){
						TurnPercent = 60;
		}
		else if (CalculatedPosition<76){
						TurnPercent = 70;
		}
		else if (CalculatedPosition<78){
						TurnPercent = 80;
		}
...
void TurnLeftPercent(double percentage){
	...
	ServoDuty = 0.077 + percentage*0.025;
	
	TIMER_A2_PWM_DutyCycle(ServoDuty,1);
	//myDelay(25);
	}
...
void TurnRightPercent(double percentage){
	...
	if(percentage>=0 && percentage<=100){
		...
		ServoDuty = 0.075 - percentage*0.025;
